...
VarList : VarList COMMA ID {install($3->varname, declaration_type, 1);}
        ...
        | VarList COMMA MUL ID {
            if (declaration_type == INT)
                install($4->varname, INT_PTR, 1);
            else
                install($4->varname, STR_PTR, 1);
        }
        | MUL ID {
            if (declaration_type == INT)
                install($2->varname, INT_PTR, 1);
            else
                install($2->varname, STR_PTR, 1);
        }
        ;
...
OutputStmt : WRITE '(' expr ')' SEMI {$$=createTree(NONE,0,WRITE_NODE,NULL,$3,NULL,NULL);}
            ;
AsgStmt : lvalue ASSIGN expr SEMI {
...
lvalue
    : id { $$ = $1; }
    | MUL id {
        if ($2->type != INT_PTR && $2->type != STR_PTR) {
            printf("dereferencing non-pointer\n");
            exit(1);
        }

        int t = ($2->type == INT_PTR) ? INT : STRING;
        $$ = createTree(t, 0, DEREF_NODE, NULL, $2, NULL, NULL);
    }
    ;
...
expr : expr PLUS expr { 
    ...
    | AND id {if ($2->type == INT)
            ...
            $$=createTree($$->type,0,ADDR_NODE,NULL,$2,NULL,NULL);
    }
    | MUL id {if ($2->type == INT_PTR)
                    $$->type = INT;
                else if ($2->type == STR_PTR)
                    $$->type = STRING;
                else{
                    printf("dereferencing error\n");
                    exit(1);
                }
            $$=createTree($$->type,0,DEREF_NODE,NULL,$2,NULL,NULL);
