...
%token KW_BEGIN END READ WRITE PLUS MINUS MUL DIV NUM ID SEMI ASSIGN STRVAL MOD
...
VarList : VarList COMMA ID {install($3->varname, declaration_type, 1);}
        ...
        | ID LBRACK NUM RBRACK LBRACK NUM RBRACK {install2darr($1->varname,declaration_type,$3->val*$6->val,$3->val,$6->val);}
        | VarList COMMA ID LBRACK NUM RBRACK LBRACK NUM RBRACK {install2darr($3->varname,declaration_type,$5->val*$8->val,$5->val,$8->val);}
        ;
...
expr : expr PLUS expr { 
    ...
    | expr MOD expr {
        typecheck($1->type,$3->type,'a');
        $$=createTree(1,0,MOD_NODE,NULL,$1,$3,NULL);
        $$->type=INT;
    }
    | expr LT expr {
...
id: ID {
    ...
    | ID LBRACK NUM RBRACK LBRACK NUM RBRACK{
        $1->Gentry=lookup($1->varname);
        if($1->Gentry==NULL){
            printf("variable %s not declared\n",$1->varname);
            exit(1);
        }
        if ($1->Gentry->rows == 1 && $1->Gentry->cols == 1) {
        printf("%s is not a 2D array\n", $1->varname);
        exit(1);
        }
        if ($3->val < 0 || $3->val >= $1->Gentry->rows) {
            printf("Row index out of bounds for %s\n", $1->varname);
            exit(1);
        }
        if ($6->val < 0 || $6->val >= $1->Gentry->cols) {
        ...
        $$ = createTree($1->type, 0, ARRAY_NODE, NULL, rowNode, $6, NULL);
    }
