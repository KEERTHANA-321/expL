...
void generateInitCode(struct tnode * root){
    int free = getReg();
...
int generateAllocCode(struct tnode* root){
        int status=0,r1;
        for (i = 0; i <= regno; i++)
            fprintf(out, "PUSH R%d\n", i);
        status = regno;
        int free = getReg();
        struct Typetable *typeTableEntry = root->type;
    ...
    // int size = typeTableEntry->size;
    // if (size <= 0 || size > 8) {
    //     yyerror("invalid size for userdefined type\n");
    // }

    // int freeReg = getFreeReg();
    // int returnReg = getFreeReg();
    // fprintf(out, "MOV R%d, \"%s\"\n", freeReg, "Alloc");
    // fprintf(out, "PUSH R%d\n", freeReg);
    // fprintf(out, "MOV R%d, %d\n", freeReg, size);
    // fprintf(out, "PUSH R%d\n", freeReg);
